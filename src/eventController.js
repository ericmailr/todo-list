...
const eventController = (() => {
  ...
  const init = () => {
    ...
    displayController.formButton.addEventListener(
      'click',
      displayController.toggleForm,
    );
    displayController.cancelButton.addEventListener(
      'click',
      displayController.toggleForm,
    );
    displayController.createTodoButton.addEventListener('click', submitTodo);
    displayController.cancelEditButton.addEventListener(
      'click',
      displayController.toggleEditForm,
    );
    displayController.projectSelectorButton.addEventListener(
      'click',
      displayController.toggleProjects,
    );

    document.addEventListener('click', function() {
      if (
        event.target != document.getElementById('projects') &&
        event.target != displayController.projectSelectorButton &&
        event.target != document.getElementById('new-project-input')
      ) {
  ...
  const addRemoveTodoEvent = todoItem => {
    document
      ...
      .lastChild.lastChild.addEventListener('click', function() {
  ...
  const addEditTodoEvent = todoItem => {
    document
      .getElementById(todoItem.id)
      .nextSibling.firstElementChild.lastElementChild.addEventListener(
        'click',
        function addEditListener() {
          ...
          displayController
            ...
            .addEventListener('click', function() {
              displayController.toggleEditForm();
              todoItem.title = document.getElementById(
                'edit-title-input',
              ).value;
              todoItem.description = document.getElementById(
                'edit-description-input',
              ).value;
              todoItem.dueDate = document.getElementById(
                'edit-due-date-input',
              ).value;
              todoItem.priority = document.getElementById(
                'edit-priority-input',
              ).value;
              displayController.renderTodo(todoItem);
              addRemoveTodoEvent(todoItem);
  ...
  const setProjectEventListeners = projectItem => {
    document
      .getElementById(`project${projectItem.id}`)
      .addEventListener('click', function() {
  ...
  const setDeleteProjectListeners = () => {
    Array.from(document.getElementsByClassName('delete-project')).forEach(
      function(deleteButton) {
        deleteButton.addEventListener('click', function() {
          event.stopPropagation();
          let parentIdNumber = parseInt(
            deleteButton.parentElement.id.substring(7),
          );
  ...
  const addPlaceholders = todoItem => {
    ...
    document.getElementById('edit-due-date-input').value =
      todoItem.dueDate == ''
        ? ''
        : new Date(todoItem.dueDate).toISOString().substring(0, 10);
